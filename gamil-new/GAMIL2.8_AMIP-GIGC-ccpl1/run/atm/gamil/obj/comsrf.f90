# 1 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/atm/GAMIL2.0_128x60/src/physics/cam1/comsrf.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/atm/GAMIL2.0_128x60/src/physics/cam1/comsrf.F90"

# 1 "./misc.h" 1
# 2 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/atm/GAMIL2.0_128x60/src/physics/cam1/comsrf.F90" 2

# 1 "./params.h" 1
# 15 "./params.h"
 
 
# 3 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/atm/GAMIL2.0_128x60/src/physics/cam1/comsrf.F90" 2
...
module comsrf
    ...
    use infnan,       only: inf, nan, uninit_r8
    ...
    real(r8), allocatable :: rho(:,:)           ! surface air density
    ...
    type(srfflx_parm), allocatable :: srfflx_parm2d_sice(:)
    type(srfflx_parm), allocatable :: srfflx_parm2d_lsm(:)
...
!
    ...
    subroutine initialize_comsrf
        ...
        allocate(rho(pcols,begchunk:endchunk))
        ...
        allocate(srfflx_parm2d_sice(begchunk:endchunk))
        allocate(srfflx_parm2d_lsm(begchunk:endchunk))
...
!$OMP PARALLEL DO PRIVATE (c)
    ...
    subroutine update_srf_fluxes(state, parm, frac, ts_to_lwup)
...
!$OMP PARALLEL DO PRIVATE (c, ncol, i,m)
        do c = begchunk, endchunk
            ncol = get_ncols_p(c)
            do i = 1, ncol
                if (frac(i,c) > 0.) then
                    ...
                    !
                    if (present(ts_to_lwup) .and. ts_to_lwup) then
                        if (frac(i,c) .ne. 1.) then
                            state(c)%lwup(i) = stebol*(state(c)%ts(i)**4)
                        end if
                    else
                        if (frac(i,c) .ne. 1.) then
...
!$OMP PARALLEL DO PRIVATE(j, ncol, i)
