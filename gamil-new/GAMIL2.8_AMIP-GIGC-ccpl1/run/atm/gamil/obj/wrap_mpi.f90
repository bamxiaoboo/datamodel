# 1 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/atm/GAMIL2.0_128x60/src/control/wrap_mpi.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/atm/GAMIL2.0_128x60/src/control/wrap_mpi.F90"

# 1 "./misc.h" 1
# 2 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/atm/GAMIL2.0_128x60/src/control/wrap_mpi.F90" 2
!---------------------------------------------------------------------------
!
! Purpose:
!
...
! 
   ...
   real (r8), intent(in)  :: sendbuf(*)
   real (r8), intent(out) :: recvbuf(*)
   integer, intent(in) :: sdispls(npes)
   integer, intent(in) :: sendcnts(npes)
   integer, intent(in) :: sendtype
   integer, intent(in) :: recvcnts(npes)
   integer, intent(in) :: rdispls(npes)
   integer, intent(in) :: recvtype
   integer, intent(in) :: comm

   integer i, status(MPI_STATUS_SIZE)
   integer :: isend(npes), irecv(npes)
   integer ier   !MP error code
 
   call t_startf ('mpi_alltoallv')

    do i = 1, npes
        if (sendcnts(i) > 0) then
            call mpi_isend(sendbuf(1+sdispls(i)), &
                sendcnts(i), sendtype, i-1, iam*npes+i-1, comm, isend(i), ier)
        endif
    enddo

    do i = 1, npes
        if (recvcnts(i) > 0) then
            call mpi_irecv(recvbuf(1+rdispls(i)), &
                recvcnts(i), recvtype, i-1, (i-1)*npes+iam, comm, irecv(i), ier)
        endif
    enddo

    do i = 1, npes
        if (sendcnts(i) > 0) then
            call MPI_WAIT(isend(i),status,ier)                         
        endif
    enddo
    do i = 1, npes
        if (recvcnts(i) > 0) then
            call MPI_WAIT(irecv(i),status,ier)                         
        endif
    enddo
...
! If  is not turned on
!
# 662 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/atm/GAMIL2.0_128x60/src/control/wrap_mpi.F90"
