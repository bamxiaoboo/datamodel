# 1 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/sice/CSIM4/ice_data.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/sice/CSIM4/ice_data.F90"

# 1 "./misc.h" 1
# 2 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/sice/CSIM4/ice_data.F90" 2

# 1 "./params.h" 1
# 15 "./params.h"
 
 
# 3 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/sice/CSIM4/ice_data.F90" 2
!----------------------------------------------------------------------- 
!
! BOP
!
...
# 124 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/sice/CSIM4/ice_data.F90" 2
...
# 125 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/sice/CSIM4/ice_data.F90" 2
!---------------------------Local variables-----------------------------
...
!!    real(r8) divdampn         ! Number of days to invoke divergence damper
      real(r8) precc_thresh     ! Precipitation threshold for PRECCINT and PRECCFRQ
      real(r8) precl_thresh     ! Precipitation threshold for PRECLINT and PRECLFRQ
# 415 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/sice/CSIM4/ice_data.F90" 2
...
!-----------------------------------------------------------------------
      ...
      integer ncid_sst   ! sst dataset
      integer luhrest    ! history restart unit
# 416 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/sice/CSIM4/ice_data.F90" 2
!---------------------------Local variables-----------------------------
  integer cnt3(3)        ! array of counts for each dimension
  integer strt3(3)       ! array of starting indices
...
!
     ...
     if (c_coupler_is_first_step()) previcefrac(:,:) = icefrac(:,:)
!
...
! !USES:
  ...
  use comsrf, only: snowhice, sicthk, icefrac, previcefrac,surface_state2d,tsice,srfflx_state2d
  use c_coupler_interface_mod
...
! only provided on the first timestep of a run.
  ...
  if (c_coupler_is_first_step().and. reset_csim_iceprops) then
     ...
     do i=1,ncol
        if (icefrac(i,lchnk) > 0._r8) then
	      ...
	      snowhice(i,lchnk) = 0._r8
           ...
           tsice(i,lchnk) = 0._r8
           do k=1,plevmx
              surface_state2d(lchnk)%tssub(i,k)=srfflx_state2d(lchnk)%sst(i)+Tffresh
           end do
        end if
     end do
  end do
