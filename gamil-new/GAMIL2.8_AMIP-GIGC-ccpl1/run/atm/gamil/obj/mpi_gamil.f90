# 1 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/atm/GAMIL2.0_128x60/src/dynamics/eul/mpi_gamil.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/atm/GAMIL2.0_128x60/src/dynamics/eul/mpi_gamil.F90"
module mpi_gamil
...
# 13 "/data3/work/yuxinzhu/test/gamil-new/model_platform/models/atm/GAMIL2.0_128x60/src/dynamics/eul/mpi_gamil.F90"

  integer, public,parameter :: COMM_TO_LEFT = 0
  integer, public,parameter :: COMM_TO_RIGHT = 1
  integer, public,parameter :: COMM_TO_TOP = 2
  integer, public,parameter :: COMM_TO_BOT = 3
  integer, public,parameter :: COMM_ROTATE_LEFT = 4
  integer, public,parameter :: COMM_ROTATE_RIGHT = 5
  integer, public,parameter :: PACK_FOR_PHYS = 1
  integer, public,parameter :: PACK_FOR_1D = 2
  integer, public,parameter :: NX_LON = 130
  integer, public,parameter :: NY_LAT = 60
  integer, public,parameter :: NZ_ALT = 27
  ...
  integer, public :: beglonex, ibeg0, ibeg1, ibeg2, ilbnd
  integer, public :: endlonex, iend0, iend1, iend2, ihbnd
  integer, public :: jbeg0, jbeg1, jbeg2
  integer, public :: jend0, jend1, jend2
  integer, public :: num_x_proc
  integer, public :: num_y_proc
  integer, public, allocatable :: dyn_cell_global_index(:) 
...
subroutine gamil_comm_init
    ...
    allocate(disp_2D_gamil_comm(nproc), disp_phys_2D(nproc), counts_2D_gamil_comm(nproc), lat_local_proc_id(NX_LON,NY_LAT))
...
subroutine gamil_2D_decomp
    ...
    num_comp_cost_lats = NY_LAT+INT(FLOAT(NY_LAT)*pole_lat_cost_ratio)*2
    ...
    num_Xcell = NX_LON / num_x_proc
    num_Xrem = mod(NX_LON, num_x_proc)
    num_Ycell = num_comp_cost_lats / num_y_proc
    num_Yrem = mod(NY_LAT, num_y_proc)
    ...
    jbeg0 = jbeg0 - INT(FLOAT(NY_LAT)*pole_lat_cost_ratio)
    jend0 = jend0 - INT(FLOAT(NY_LAT)*pole_lat_cost_ratio)
    if (jend0 < 1 .or. jbeg0 > NY_LAT .or. pole_lat_cost_ratio < 0) then
        write(6,*) 'error for pole_lat_cost_ratio'
        call endrun
    end if

    if (jbeg0 < 1) jbeg0 = 1
    if (jend0 > NY_LAT) jend0 = NY_LAT
    ...
    if (endlonex .gt. NX_LON) endlonex = NX_LON
    ilbnd = beglonex
    ihbnd = endlonex
    beglatex = jbeg0 - 1
    endlatex = jend0 + 1
...
!    call mpi_gatherv(count,1,MPI_INTEGER,counts_2D_gamil_comm,counts_2D_gamil_comm,disps_1,MPI_INTEGER,rootproc_id,gamil_comm,ierr)
    ...
    if (disp_2D_gamil_comm(1)+counts_2D_gamil_comm(1) .ne. NX_LON*NY_LAT) then
       write(6,*) 'error in decomp', disp_2D_gamil_comm(1), counts_2D_gamil_comm(1), NX_LON*NY_LAT
       call endrun
    endif
    ...
    allocate(gather_scatter_buf1_2D(NX_LON*NY_LAT*32), gather_scatter_buf2_2D(NX_LON*NY_LAT*32))
    allocate(send_buf_left((jend0-jbeg0+1)*2*NZ_ALT*16))
    allocate(send_buf_right((jend0-jbeg0+1)*2*NZ_ALT*16))
    allocate(recv_buf_left((jend0-jbeg0+1)*2*NZ_ALT*16))
    allocate(recv_buf_right((jend0-jbeg0+1)*2*NZ_ALT*16))
    allocate(send_buf_top((iend0-ibeg0+1)*2*NZ_ALT*16))
    allocate(send_buf_bot((iend0-ibeg0+1)*2*NZ_ALT*16))
    allocate(recv_buf_top((iend0-ibeg0+1)*2*NZ_ALT*16))
    allocate(recv_buf_bot((iend0-ibeg0+1)*2*NZ_ALT*16))
...
!    write(6,*) 'proc physics', myproc_id, left_procid, right_procid, top_procid, bot_procid
    ...
    do i=1, requests_size
      ...
      allocate(icomm_requests(i)%send_buffer((jend0-jbeg0+1+iend0-ibeg0+1)*2*NZ_ALT*16))
      allocate(icomm_requests(i)%recv_buffer((jend0-jbeg0+1+iend0-ibeg0+1)*2*NZ_ALT*16))
    end do

    allocate(dyn_cell_global_index((ihbnd-ilbnd+1)*(endlatexdyn-beglatexdyn+1)))
    dyn_cell_global_index=0
    do j=jbeg0,jend0
       do i=ibeg0,iend0
          dyn_cell_global_index((i-ilbnd+1)+(j-beglatexdyn)*(ihbnd-ilbnd+1))=i+(NX_LON-2)*(j-1)
          if (i .gt. NX_LON-2) then
             dyn_cell_global_index((i-ilbnd+1)+(j-beglatexdyn)*(ihbnd-ilbnd+1))=i-(NX_LON-2)+(NX_LON-2)*(j-1)
          end if
       end do
    end do
...
subroutine gamil_scatter_3D_array_phys(array_g, ibeg, iend, jbeg,jend, nlevels, array)
    implicit none
    real :: array(ibeg:iend,jbeg:jend,1:nlevels)
    real :: array_g(NX_LON,1:nlevels,NY_LAT)
    real,allocatable :: array_2d_tmp_input(:,:)
    real,allocatable :: array_2d_tmp_output(:,:)
    integer :: ibeg,iend,jbeg,jend, nlevels
    integer :: i, j, k

    allocate(array_2d_tmp_input(NX_LON,NY_LAT),array_2d_tmp_output(NX_LON,NY_LAT))
    ...
    do k = 1, nlevels
      ...
      call gamil_scatter_2D_array_phys(array_2d_tmp_input, 1, NX_LON, 1, NY_LAT, array_2d_tmp_output)
      
      do j = jbegs_phys(myproc_id+1), jends_phys(myproc_id+1)
      do i = ibeg0, iend0
         array(i,j,k) = array_2d_tmp_output(i,j)
      enddo
      enddo
    enddo
...
subroutine gamil_gather_2D_array_phys(array_g, ibeg, iend, jbeg,jend, array)
    ...
    if (is_rootproc) then
       do k=1, nproc
          do j=jbegs_phys(k), jends_phys(k)
             do i=ibegs(k), iends(k)
                array_g(i,j) = gather_scatter_buf2_2D((j - jbegs_phys(k))*(iends(k)-ibegs(k)+1)+i-ibegs(k)+1+disp_phys_2D(k))
             enddo
          enddo
       enddo
    endif
...
subroutine gamil_gather_3D_array_phys(array_g, ibeg, iend, jbeg,jend, nlevels, array)
    ...
    do k = 1, nlevels
      do j = jbegs_phys(myproc_id+1), jends_phys(myproc_id+1)
      do i = ibeg0, iend0
         array_2d_tmp_output(i,j) = array(i,j,k)
      enddo
      enddo

      call gamil_gather_2D_array_phys(array_2d_tmp_input, 1, NX_LON, 1, NY_LAT, array_2d_tmp_output)
...
subroutine gamil_scatter_3D_array(array_g, ibeg, iend, jbeg, jend, kbeg, kend, array)
    ...
    real :: array_g(NX_LON,NY_LAT,kbeg:kend)
...
subroutine gamil_3D_array_pack(pack_direction,ibeg,iend,jbeg,jend,kbeg,kend,iter,nlevels,arr,pack_mode,buffer)
    ...
    if (pack_direction .eq. 0) then
    ...
    else if (pack_direction .eq. 1) then
      local_jbeg = jbeg0
      local_jend = jend0
      if (pack_mode .eq. 1) then
        local_jbeg = jbegs_phys(myproc_id+1)-1
    ...
    else if (pack_direction .eq. 2) then
      local_jbeg = jbeg0
      if (pack_mode .eq. 1) then
      ...
      if (iend0 .lt. NX_LON) local_iend = local_iend+1
      if (jbeg .eq. jend) then
    ...
    else if (pack_direction .eq. 3) then
      local_jend = jend0
      if (pack_mode .eq. 1) then
...
subroutine gamil_3D_array_unpack(unpack_direction,ibeg,iend,jbeg,jend,kbeg,kend,iter,nlevels,arr,pack_mode,buffer)
    ...
    if (unpack_direction .eq. 4) then
    ...
    else if (unpack_direction .eq. 5) then
      local_jbeg = jbeg0
      local_jend = jend0
      if (pack_mode .eq. 1) then
        local_jbeg = jbegs_phys(myproc_id+1)-1
      ...
      local_iend = endlonex
    else if (unpack_direction .eq. 6) then
      local_jbeg = jbeg0
      if (pack_mode .eq. 1) then
        local_jbeg = jbegs_phys(myproc_id+1)
      endif
    ...
    else if (unpack_direction .eq. 7) then
      local_jend = jend0
      if (pack_mode .eq. 1) then
        local_jend = jends_phys(myproc_id+1)
      endif
...
subroutine unpack_request_comm_arrays(rid)
    ...
    iter = 1
    unpack_direction = -1
    if ((endlonex .eq. NX_LON .and. icomm_requests(rid)%comm_direction .eq. COMM_ROTATE_LEFT) .or. &
    ...
    if (unpack_direction .eq. -1) return
...
subroutine gamil_all_reduce(value1,value2,value3,value4)
    ...
    call c_coupler_allreduce_real16(values,results,num_values,gamil_comm,nproc)
...
subroutine gamil_average_pole_data_phys(lat_indx,ibeg,iend,lev,array1,array2)
    ...
    local_ibeg = ibeg0
    local_iend = iend0
    if (iend0 .eq. NX_LON) local_iend = iend0 - 2
...
!$OMP PARALLEL do private(i,k,tmp_sum)
    ...
    if (present(array2)) then
       call c_coupler_allreduce_real16(local_sum,global_sum,lev*2,lat_comm,num_x_proc)
    else 
       call c_coupler_allreduce_real16(local_sum,global_sum,lev,lat_comm,num_x_proc)
    endif
    call t_stopf("dyn polereduce")

!$OMP PARALLEL do private(k,i,tmp_sum)
    do k=1,lev
       tmp_sum = global_sum(k) / dble(NX_LON-2)
       do i=ibeg, iend
          array1(i,k) = tmp_sum 
       enddo
    enddo
...
!$OMP PARALLEL do private(k,i,tmp_sum)
    do k=1,lev
       tmp_sum = global_sum(k+lev) / dble(NX_LON-2)
       do i=ibeg, iend
          array2(i,k) = tmp_sum 
       enddo
    enddo
...
subroutine gamil_sum_pole_data_phys(lat_indx,array1,num_data1,array2,num_data2)
    ...
    integer :: lat_indx
    real*16 :: array1(:)
    real*16 :: tmp_array1(NZ_ALT*16), tmp_array2(NZ_ALT*16)
    ...
    call t_startf("dyn polereduce")
    call c_coupler_allreduce_real16(tmp_array1,tmp_array2,iter,lat_comm,num_x_proc)
    call t_stopf("dyn polereduce")
...
subroutine gamil_min_lat_row_data(array1,num_data1,array2,num_data2)
    ...
    integer :: num_data1
    real*8 :: array1(:)
    real*8 :: tmp_array1((NZ_ALT+NY_LAT)*16), tmp_array2((NZ_ALT+NY_LAT)*16)
...
subroutine endrun()
   use c_coupler_interface_mod
   call c_coupler_abort("gamil call endrun")
