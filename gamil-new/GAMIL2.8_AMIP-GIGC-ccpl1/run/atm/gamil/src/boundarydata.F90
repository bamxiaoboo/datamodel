      ...
      !! wrap_inq_var (nfid, varid, varname, xtype, ndims, dimids, natts)
       ...
       do id=1,bndydata%ndims
          if(bndydata%dimids(id).eq.londimid) then
             ...
             print *, __LINE__,fieldnames(i),id,bndydata%dimids(id),londimid, &
...
35     continue
  ...
  subroutine ncol_read_bracket(cid,columnmap,start,count,ncol)
    ...
    print *, __LINE__,cid(1),cid(ncol),minval(cid(1:ncol)),maxval(cid(1:ncol))
