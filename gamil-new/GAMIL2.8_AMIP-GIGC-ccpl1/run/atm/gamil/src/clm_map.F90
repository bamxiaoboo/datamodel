    ...
    if (gchnk /= -1) then
        ...
        do m = 1, ncols
            ...
            if (landmask(i, j) == 1) then
                ...
                do n = 1, maxpatch
                    if (wtxy(i,j,n) > 0.) then                
                        landvec%patch(proc_land_count(k),n) = proc_patch_count(k)  !land subgrid patch number
                        landvec%wtxy(proc_land_count(k), n) = wtxy(i,j,n)   !land subgrid weights
                        patchvec%ixy(proc_patch_count(k)) = i             !patch longitude index
                        patchvec%jxy(proc_patch_count(k)) = j             !patch latitude index
                        patchvec%mxy(proc_patch_count(k)) = n             !patch subgrid index of lnd point
                        patchvec%wtxy(proc_patch_count(k)) = wtxy(i,j,n)   !patch weight
...
! Error check: make sure weights sum to one for each land cell
  ...
  if (masterproc) then
      do i=1,numpatch
        write (6,*) "[ZZ]", patchvec%ixy(i), patchvec%jxy(i), patchvec%mxy(i), &
        patchvec%wtxy(i)
      enddo
  endif
#endif
