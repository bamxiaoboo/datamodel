...
{
	EXECUTION_REPORT(REPORT_ERROR, ensemble_member_id > 0, "The number of an ensemble member must be a positive integer\n");
	EXECUTION_REPORT(REPORT_LOG, true, "Ensemble is enabled. The ensemble member id is %d", ensemble_member_id);
	...
	if (have_random_seed_for_perturbation) {
		...
		else EXECUTION_REPORT(REPORT_ERROR, false, "\"%s\" is not a right selection for perturbing the roundoff errors of the registerred fields. Existing selections include set_last_bit_to_1, set_last_bit_to_0, reverse_last_bit and xor_last_bit_with_a_bit", perturbation_type);
		...
		EXECUTION_REPORT(REPORT_LOG, true, "In the ensemble experiment of perturbing roundoff errors, for the ensemble member %d, the seed of random number generation is %d", ensemble_member_id, this->ensemble_random_seed_for_perturbation);
	}
}
...
void Ensemble_mgt::register_a_field_for_perturbation(void *data_buf)
{
	...
	EXECUTION_REPORT(REPORT_ERROR, registered_field != NULL && registered_field->get_is_registered_model_buf(), 
					 "The field data buffer for perturbing the roundoff errors has not been registerred to C-Coupler before. Please check.");
	EXECUTION_REPORT(REPORT_ERROR, words_are_the_same(registered_field->get_field_data()->get_grid_data_field()->data_type_in_application, DATA_TYPE_FLOAT) || words_are_the_same(registered_field->get_field_data()->get_grid_data_field()->data_type_in_application, DATA_TYPE_DOUBLE),
...
void Ensemble_mgt::perturb_a_field_through_set_last_bit_to_1(void *field_data_buf, const char *data_type, long field_size, int current_random_number)
{
	...
	else EXECUTION_REPORT(REPORT_ERROR, false, "C-Coupler software error in perturb_a_field_through_set_last_bit_to_1");
}
...
void Ensemble_mgt::perturb_a_field_through_set_last_bit_to_0(void *field_data_buf, const char *data_type, long field_size, int current_random_number)
{
	...
	else EXECUTION_REPORT(REPORT_ERROR, false, "C-Coupler software error in perturb_a_field_through_set_last_bit_to_0");
}
...
void Ensemble_mgt::perturb_a_field_through_reverse_last_bit(void *field_data_buf, const char *data_type, long field_size, int current_random_number)
{
	if (words_are_the_same(data_type, DATA_TYPE_FLOAT))
		...
		for (long i = 0; i < field_size; i ++) 
			((long*) field_data_buf)[i] = ((long*) field_data_buf)[i] ^ ((long)1);
	else EXECUTION_REPORT(REPORT_ERROR, false, "C-Coupler software error in perturb_a_field_through_reverse_last_bit");
}
