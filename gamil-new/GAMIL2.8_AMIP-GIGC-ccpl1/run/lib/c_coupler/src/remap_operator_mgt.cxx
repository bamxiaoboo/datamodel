...
void Remap_operator_mgt::execute(const char*function, Remap_statement_operand **statement_operands, int num_operands)
{
    ...
    if (words_are_the_same(function, FUNCTION_WORD_NEW_OPERATOR)) {
        ...
        if (num_remap_grids == 2) {
            ...
            EXECUTION_REPORT(REPORT_ERROR, num_leaf_grids_src == num_leaf_grids_dst,
        ...
        EXECUTION_REPORT(REPORT_ERROR, num_operands == 3, "function \"%s\" must have three input parameters\n", function);
    ...
    else EXECUTION_REPORT(REPORT_ERROR, false, "function \"%s\" is not support in the management of remap operators\n", function);
}
