...
long cpl_get_grid_size(const char *grid_name)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, model_grid != NULL, "grid %s is undefined\n", grid_name);
    EXECUTION_REPORT(REPORT_ERROR, model_grid->get_grid_size() > 0, "the grid size %%s is not known\n", grid_name);
...
double *cpl_get_sphere_grid_center_fields(const char *grid_name, const char *coord_label)
{
    ...
    model_grid = remap_grid_manager->search_remap_grid_with_grid_name(grid_name);    
    EXECUTION_REPORT(REPORT_ERROR, model_grid != NULL, "grid %s is undefined\n", grid_name);
    EXECUTION_REPORT(REPORT_ERROR, model_grid->get_grid_size() > 0, "the grid size of %s is not known\n", grid_name);
    EXECUTION_REPORT(REPORT_ERROR, model_grid->get_is_sphere_grid(), "%s is not a sphere grid\n", grid_name);
    EXECUTION_REPORT(REPORT_ERROR, model_grid->has_grid_coord_label(coord_label), "%s is not a right coordinate label, which should be lat or lon\n", coord_label);
    ...
    EXECUTION_REPORT(REPORT_ERROR, center_field != NULL, "the center values of %s in grid %s have not been defined\n", coord_label, grid_name);
...
double *cpl_get_sphere_grid_area(const char *grid_name)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, model_grid->get_grid_size() > 0, "the grid size of %s is not known\n", grid_name);
    EXECUTION_REPORT(REPORT_ERROR, model_grid->get_is_sphere_grid(), "%s is not a sphere grid\n", grid_name);
    EXECUTION_REPORT(REPORT_ERROR, model_grid->get_area_or_volumn() != NULL, "the area of grid %s is unknown\n", grid_name);
...
bool *cpl_get_sphere_grid_mask(const char *grid_name)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, model_grid->get_grid_size() > 0, "the grid size of %s is not known\n", grid_name);
    EXECUTION_REPORT(REPORT_ERROR, model_grid->get_is_sphere_grid(), "%s is not a sphere grid\n", grid_name);
    EXECUTION_REPORT(REPORT_ERROR, model_grid->get_grid_mask_field() != NULL, "the mask of grid %s is unknown\n", grid_name);
...
long cpl_get_num_levs_in_grid(const char *grid_name)
{
    Remap_grid_class *leaf_grids[256], *grid;
    int num_leaf_grids;


    grid = remap_grid_manager->search_remap_grid_with_grid_name(grid_name);
    EXECUTION_REPORT(REPORT_ERROR, grid->get_num_dimensions() == 3, "C-Coupler software error1 in cpl_get_num_levs_in_grid\n");
    
    grid->get_leaf_grids(&num_leaf_grids, leaf_grids, grid);
    EXECUTION_REPORT(REPORT_ERROR, words_are_the_same(leaf_grids[2]->get_coord_label(), COORD_LABEL_LEV), "C-Coupler software error2 in cpl_get_num_levs_in_grid\n");
...
Remap_grid_data_class *cpl_duplicate_field_with_double_data_type(Remap_grid_data_class *float_field)
{
	...
	EXECUTION_REPORT(REPORT_ERROR, float_field->get_coord_value_grid()->get_grid_size() == double_field->get_grid_data_field()->required_data_size,
				 "C-Coupler software error in cpl_duplicate_field_with_double_data_type\n");
	EXECUTION_REPORT(REPORT_LOG, true, "cpl duplicated field size is %ld", double_field->get_grid_data_field()->required_data_size);
