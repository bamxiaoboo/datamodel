                                             ...
                                             const char *grid_data_type)
{
    ...
    if (grid_name != NULL) {
        ...
        EXECUTION_REPORT(REPORT_ERROR, words_are_the_same(remap_grid_manager->search_remap_grid_with_grid_name(grid_name)->get_coord_label(), field_data_name),
    ...
    if (words_are_the_same(span_data_type, DATA_TYPE_LONG)) {
        EXECUTION_REPORT(REPORT_ERROR, (long) bound_start_value == bound_start_value, "the first input parameter of ispan function must be an integer\n");
        EXECUTION_REPORT(REPORT_ERROR, (long) bound_end_value == bound_end_value, "the second input parameter of ispan function must be an integer\n");
        EXECUTION_REPORT(REPORT_ERROR, ((long)bound_end_value - (long)bound_start_value)%(num_value_points-1) == 0, "for ispan function, the number of elements in span array does not match the start and end boundaries\n");
    }
    else EXECUTION_REPORT(REPORT_ERROR, words_are_the_same(span_data_type, DATA_TYPE_DOUBLE), "remap software error2 in Remap_grid_data_class for spanning data\n");
    ...
    else EXECUTION_REPORT(REPORT_ERROR, false, "remap software error3 in Remap_grid_data_class for spanning data\n");
}
...
void Remap_grid_data_class::generate_grid_info(Remap_grid_class *coord_value_grid)
{
    ...
    if (coord_value_grid != NULL) {
        ...
        EXECUTION_REPORT(REPORT_ERROR, this->sized_grids.size() > 0, "remap software error in function Remap_grid_data_class\n");
    }
}
...
void Remap_grid_data_class::interchange_grid_data(Remap_grid_class *interchange_grid)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, src_grid_of_grid_data->is_similar_grid_with(interchanged_grid_of_grid_data), 
...
void Remap_grid_data_class::reset_sized_grids(int num_sized_grids, Remap_grid_class **sized_grids)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, grid_data_field->required_data_size%grid_size == 0, "remap software error2 in reset_sized_grids\n");    
}
...
bool Remap_grid_data_class::is_unit_degree()
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, i < num_leaf_grids, "remap software error1 in is_unit_degree\n");
    EXECUTION_REPORT(REPORT_ERROR, !words_are_the_same(leaf_grids[i]->get_coord_unit(), COORD_UNIT_RADIANS), "remap software error2 in is_unit_degree\n");
...
void Remap_grid_data_class::generate_analytic_values(const char *case_name)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, coord_value_grid != NULL,
                 "the field data used to generate analytic values must have associated grid\n");
    EXECUTION_REPORT(REPORT_ERROR, words_are_the_same(grid_data_field->data_type_in_application, DATA_TYPE_DOUBLE),
                 "the data type of the field data used to generate analytic values must be double\n");
    EXECUTION_REPORT(REPORT_ERROR, grid_data_field->required_data_size == coord_value_grid->get_grid_size(),
                 "remap software error in generate_analytic_values\n");
    EXECUTION_REPORT(REPORT_ERROR, coord_value_grid->get_num_dimensions() == 2, "can only generate the analytic values of 2D grid\n");

    coord_value_grid->get_leaf_grids(&num_leaf_grids, leaf_grids, coord_value_grid);
    for (i = 0; i < num_leaf_grids; i ++)
        EXECUTION_REPORT(REPORT_ERROR, leaf_grids[i]->get_super_grid_of_setting_coord_values() != NULL, 
    ...
    if (words_are_the_same(case_name, "analytic_2D_case1")) {
        EXECUTION_REPORT(REPORT_ERROR, coord_value_grid->get_num_dimensions() == 2,
    ...
    else EXECUTION_REPORT(REPORT_ERROR, false, "\"%s\" is not a legal case name of generating analytic values\n", case_name);
...
void Remap_grid_data_class::evaluate_error(Remap_grid_data_class *first_field_data, Remap_grid_data_class *second_field_data)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, first_field_data->coord_value_grid != NULL && first_field_data->coord_value_grid->is_similar_grid_with(second_field_data->coord_value_grid), 
                 "the two field data used for evaluation must have similar associated grids\n");
    EXECUTION_REPORT(REPORT_ERROR, first_field_data->have_data_content(),
    ...
    EXECUTION_REPORT(REPORT_ERROR, second_field_data->have_data_content(),
                 "field data \"%s\" does not have essential data content. It can not be used for evaluation\n",
                 second_field_data->grid_data_field->field_name_in_application);
    EXECUTION_REPORT(REPORT_ERROR, words_are_the_same(first_field_data->grid_data_field->data_type_in_application, second_field_data->grid_data_field->data_type_in_application) &&
