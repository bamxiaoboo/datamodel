...
void Remap_operator_basis::register_remap_grids(int num_remap_grids, Remap_grid_class **remap_grids)
{
    ...
    if (is_operator_regriding)
        EXECUTION_REPORT(REPORT_ERROR, num_remap_grids == 2, 
        ...
        EXECUTION_REPORT(REPORT_ERROR, num_remap_grids == 1, 
    ...
    for (i = 0; i < num_remap_grids; i ++) {
        EXECUTION_REPORT(REPORT_ERROR, remap_grids[i]->get_num_dimensions() == num_dimensions, 
        ...
        EXECUTION_REPORT(REPORT_ERROR, remap_grids[i]->get_grid_size()> 0, 
        ...
        EXECUTION_REPORT(REPORT_ERROR, !remap_grids[i]->is_partial_grid(),
    ...
    if (num_remap_grids == 2) {
        ...
        for (i = 0; i < num_leaf_grids; i ++)
            EXECUTION_REPORT(REPORT_ERROR, words_are_the_same(leaf_grids_src[i]->get_coord_label(), leaf_grids_dst[i]->get_coord_label()),
...
{
    long src_cell_neighbors_indexes[256];
    int num_src_cell_neighbors, i;
    bool cell_mask_src;


    if (!visit_cell_in_src_grid(cell_index_src))
        return;
    
    if (!src_cell_and_dst_cell_have_overlap(cell_index_src, cell_index_dst))
        return;

    get_cell_mask_of_src_grid(cell_index_src, &cell_mask_src);
    if (cell_mask_src) 
        overlapping_src_cells_indexes[num_overlapping_src_cells++] = cell_index_src;

	if (num_overlapping_src_cells >= 409600) {
		for (i = 0; i < 409600; i ++) {
			printf("%ld error overlap with %ld\n", cell_index_dst, overlapping_src_cells_indexes[i]);
		int num_vertexes_src;
		double vertex_coord_values_src[256*2];
		double cell_bounding_box_src[256*2];
    	get_cell_vertex_coord_values_of_src_grid(overlapping_src_cells_indexes[i], &num_vertexes_src, vertex_coord_values_src, true);
		compute_cell_bounding_box(num_vertexes_src, 2, vertex_coord_values_src, cell_bounding_box_src);
		printf("critical src cell: " );
		for (int i = 0; i < num_vertexes_src; i ++)
			printf("<%lf %lf>  ", vertex_coord_values_src[2*i], vertex_coord_values_src[2*i+1]);
		printf("\n");
		printf("bounding box is <%lf %lf> <%lf %lf>\n", cell_bounding_box_src[0],cell_bounding_box_src[1], cell_bounding_box_src[2], cell_bounding_box_src[3]);			
		}
		int num_vertexes_dst;
		double vertex_coord_values_dst[256*2];
		double cell_bounding_box_dst[256*2];
    	get_cell_vertex_coord_values_of_dst_grid(cell_index_dst, &num_vertexes_dst, vertex_coord_values_dst, true);
		compute_cell_bounding_box(num_vertexes_dst, 2, vertex_coord_values_dst, cell_bounding_box_dst);
		printf("critical dst cell: " );
		for (int i = 0; i < num_vertexes_dst; i ++)
			printf("<%lf %lf>  ", vertex_coord_values_dst[2*i], vertex_coord_values_dst[2*i+1]);
		printf("\n");
		printf("bounding box is <%lf %lf> <%lf %lf>\n", cell_bounding_box_dst[0],cell_bounding_box_dst[1], cell_bounding_box_dst[2], cell_bounding_box_dst[3]);
	}
    EXECUTION_REPORT(REPORT_ERROR, num_overlapping_src_cells < 409600, "remap software error in get_all_overlapping_src_cells_recursively");
    
    get_cell_neighbors_in_src_grid(cell_index_src, &num_src_cell_neighbors, src_cell_neighbors_indexes);
    for (i = 0; i < num_src_cell_neighbors; i ++)
        get_all_overlapping_src_cells_recursively(cell_index_dst, src_cell_neighbors_indexes[i], num_overlapping_src_cells, overlapping_src_cells_indexes);
}
    ...
    for (cell_index_dst = 0; cell_index_dst < dst_grid->get_grid_size(); cell_index_dst ++) {
        ...
        for (i = 0; i < num_vertexes_dst; i ++) {
            if (vertex_coord_values_dst[i*num_grid_dimensions_dst] == NULL_COORD_VALUE)
                continue;
            search_cell_in_src_grid(vertex_coord_values_dst+i*num_grid_dimensions_dst, &cell_index_src, false);
            if (cell_index_src != -1)
                break;
        }    
        if (cell_index_src == -1)
        ...
        num_overlapping_src_cells = 0;
        get_all_overlapping_src_cells_recursively(cell_index_dst, cell_index_src, num_overlapping_src_cells, overlapping_src_cells_indexes);
