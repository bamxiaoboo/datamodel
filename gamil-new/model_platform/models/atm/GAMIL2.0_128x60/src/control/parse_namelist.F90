...
subroutine parse_namelist
    ...
    ! *** added by DONG Li *** !
#if (defined TRIAL_RUN_STRATIFORM)
    use StratiformSchemeDriver, only: stratiform_scheme
#endif
#if (defined TRIAL_RUN_FORCING)
    use ForcingScenario
    use SolarForcing,    only: SolarForcing_scenario => scenario, &
                               SolarForcing_datafile => datafile
    use GHGForcing,      only: GHGForcing_scenario => scenario, &
                               GHGForcing_datafile => datafile
#endif
    ! ************************ !
...
#include <comsol.h>
    ...
    namelist /atmexp/ ctitle  ,ncdata  ,bndtvs  ,bndtvo  , bndtvg , &
        ...
        reset_csim_iceprops, &
#if (defined TRIAL_RUN_STRATIFORM)
        ideal_phys, convection_scheme, stratiform_scheme, & ! physical package
#else
        ideal_phys, convection_scheme, & ! physical package
#endif
#if (defined TRIAL_RUN_FORCING)
        SolarForcing_datafile, &
        GHGForcing_datafile,   &
#endif
...
#if (defined TRIAL_RUN_STRATIFORM)
    if (stratiform_scheme /= "RK" .and. stratiform_scheme /= "MG") then
        write(6, "('Error: parse_namelist: input namelist ')", advance="no")
        write(6, "('""stratiform_scheme"" must be set to either RK or MG')")
        call endrun
    end if
#else
    ...
    if (scenario_ghg == "FIXED") then
        GHGForcing_scenario = FixedScenario
    else if (scenario_ghg == "RAMPED") then
        GHGForcing_scenario = RampedScenario
    else
        write(*, "('Error: parse_namelist: In namelist, ')", advance="no")
        write(*, "('""scenario_ghg"" must be set to FIXED, RAMPED or IPCC')")
        call endrun
    end if
#else
    if (scenario_ghg == 'FIXED') then
    ...
    if (scenario_scon == 'FIXED') then
        SolarForcing_scenario = FixedScenario
    else if (scenario_scon == 'RAMPED') then
        SolarForcing_scenario = RampedScenario
    else
        write(6, "('Error: parse_namelist: input namelist ""scenario_scon""')", advance="no")
        write(6, "(' must be set to either FIXED or RAMPED.')")
        call endrun
    end if
#else
   if (scenario_scon == 'FIXED') then
