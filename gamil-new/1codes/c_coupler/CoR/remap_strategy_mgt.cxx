...
void Remap_strategy_mgt::execute(const char*function, Remap_statement_operand **statement_operands, int num_operands)
{
    ...
    if (words_are_the_same(function, FUNCTION_WORD_COMBINE_OPERATORS)) {
        EXECUTION_REPORT(REPORT_ERROR, num_operands >= 2, "function \"%s\" must have one result parameter and at least two input parameters\n", function);
        ...
        EXECUTION_REPORT(REPORT_ERROR, num_operands == 3, "function \"%s\" must have non result parameter and three input parameters\n", function);
